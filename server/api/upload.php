...
require_once __DIR__ . '/../lib/image_processor.php';
...
if ($uploadedFile['error'] !== UPLOAD_ERR_OK) {
    ...
    jsonError('Upload gagal: ' . ($phpUploadErrors[$uploadedFile['error']] ?? 'Unknown error'));
}
...
// Validasi MIME type (lebih aman dari cek ekstensi)
$mimeType = mime_content_type($uploadedFile['tmp_name']);
if (!array_key_exists($mimeType, MIME_TO_EXT)) {
    jsonError("File bukan gambar yang didukung. MIME terdeteksi: $mimeType");
}

$originalExt  = MIME_TO_EXT[$mimeType];
...
// =============================================
//  LOAD & CONVERT GAMBAR (via lib/image_processor.php)
// =============================================

$sourceImage = loadSourceImage($tmpPath, $mimeType);
...
$convertedName = "{$baseName}_" . date('Ymd_His') . '_' . substr(uniqid(), -6) . ".{$targetFormat}";
$outputPath    = UPLOAD_DIR . $convertedName;

if (!saveConvertedImage($sourceImage, $outputPath, $targetFormat)) {
...
try {
    ...
    // DB error tidak blokir response — file sudah jadi, tetap return sukses
